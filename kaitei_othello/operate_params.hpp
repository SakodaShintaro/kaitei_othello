...
#else
void regularize(EvalParams<DefaultEvalType>& eval_params) {
    for (int k = 0; k < SqNum; k++) {
        for (int p1 = 0; p1 < PieceStateNum; p1++) {
            for (int p2 = 0; p2 < PieceStateNum; p2++) {
                if (p1 == p2) {    //kppt‚Ìp1 == p2‚É‚Â‚¢‚Ä‚Í0‚ª“ü‚é‚æ‚¤‚É‚·‚é
                    eval_params.kpp[k][p1][p2][0] = eval_params.kpp[k][p1][p2][1] = 0;
                } else {    //kppt[k][p1][p2] == kppt[k][p2][p1]‚Æ‚È‚é‚æ‚¤‚É‚·‚é
                    DefaultEvalType mid0 = (eval_params.kpp[k][p1][p2][0] + eval_params.kpp[k][p2][p1][0]) / 2;
                    DefaultEvalType mid1 = (eval_params.kpp[k][p1][p2][1] + eval_params.kpp[k][p2][p1][1]) / 2;
...
void checkSymmetry(EvalParams<DefaultEvalType>& eval_params) {
    for (int k = 0; k < SqNum; k++) {
        for (int p1 = 0; p1 < PieceStateNum; p1++) {
            for (int p2 = 0; p2 < PieceStateNum; p2++) {
                for (int t = 0; t < 2; t++) {
...
void readAnotherFormatFile(std::string path, std::string kkp_suffix, std::string kpp_suffix) {
