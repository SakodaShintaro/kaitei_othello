...
CalcType Position::valueForTurn() {
    if (!already_calc_) {
        initScore();
    }
...
void Position::initKKP_KPPScoreBySIMD() {
    ...
    for (uint32_t i = 0; i < PIECE_STATE_LIST_SIZE; i++) {
        ...
        for (; j + 8 < PIECE_STATE_LIST_SIZE; j += 8) {
            //list[0],[1]から8要素ロード
        ...
        for (; j + 4 < PIECE_STATE_LIST_SIZE; j += 4) {
...
void Position::initFeature() {
    ...
    for (int32_t i = 0; i < PIECE_STATE_LIST_SIZE; i++) {
        piece_state_to_index_[ee_.piece_state_list[0][i]] = i;
    }
}
...
void Position::updatePieceStateList(PieceState before, PieceState after) {
    ...
    piece_state_to_index_[after] = index;
}
...
void Position::changeOnePSBySIMD(PieceState ps, int c) {
    ...
    for (; i + 4 < PIECE_STATE_LIST_SIZE; i += 4) {
        ...
        //indexesをオフセットとしてkppから4要素ギャザー
        __m128i gathred0 = _mm_i32gather_epi32(reinterpret_cast<const int*>(&eval_params_.kpp[bk_sq][ps]), indexes0, 4);
        __m128i gathred1 = _mm_i32gather_epi32(reinterpret_cast<const int*>(&eval_params_.kpp[wk_sqr][invPieceState(ps)]), indexes1, 4);
...
Score Position::SEE() {
    ...
    Move *start = move_buf, *end = move_buf;
    ...
    //std::cout << (int)best_score << std::endl;
    return best_score;
}
        ...
        if (move.capture() == EMPTY) { //玉を動かし、駒は取らない手
            ...
            for (int j = 0; j < PIECE_STATE_LIST_SIZE; j++) {
                ...
                for (; i + 4 < PIECE_STATE_LIST_SIZE; i += 4) {
                    ...
                    //indexesをオフセットとしてkppから4要素ギャザー
                    __m128i gathred = _mm_i32gather_epi32(reinterpret_cast<const int*>(&eval_params_.kpp[king_sq_num][list[j]]), indexes, 4);

                    // 16 ビット整数→32 ビット整数に変換する
                    __m256i expanded = _mm256_cvtepi16_epi32(gathred);
    ...
    for (int i = 0; i < KKP_KPP_END; i++) {
        for (int j = 0; j < RAW_TURN_END; j++) {
            if (copy[i][j] != score_state_[i][j]) {
                ...
                print();
                for (int i = 0; i < PIECE_STATE_LIST_SIZE; i++) {
                    std::cout << ee_.piece_state_list[0][i] << " " << ee_.piece_state_list[1][i] << std::endl;
                }
                assert(false);
            }
        }
