...
struct Features {
    ...
    PieceState piece_state_list[ColorNum][PIECE_STATE_LIST_SIZE]; //先手から見たもの[0] 後手からみたもの[1]を両方持つ
    Color color;
...
inline bool isEqual(Features f1, Features f2) {
    ...
    return (f1.king_sq[BLACK] == f1.king_sq[BLACK] && f2.king_sq[WHITE] == f2.king_sq[WHITE] && f1.color == f2.color);
}
